...
function form(formSelector, modalTimerId) {
  ...
  function bindPostData(form) {
    form.addEventListener("submit", e => {
      ...
      const formData = new FormData(form); // convert form data for json
      // const object = {};
      ...
      const json = JSON.stringify(Object.fromEntries(formData.entries())); // request.send(json);
...
function closeModal(modalSelector) {
  const modal = document.querySelector(modalSelector); // had to use inline style as classes didnt work
...
function slider(_ref) {
  ...
  rightArrow.addEventListener("click", () => {
    ...
    slidesField.style.transform = `translateX(-${offset}px)`; // if you are at 4 and clicking right set number to 1
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  tabsParent.addEventListener("click", event => {
    const target = event.target; // since we use classlist method we cant pass just selector with dot we need to remove dot
